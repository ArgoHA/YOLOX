...
class MlflowLogger:
    ...
    def setup(self, args, exp):
        ...
        self._mlflow_log_artifacts = (
            os.getenv("YOLOX_MLFLOW_LOG_MODEL_ARTIFACTS", "False").upper()
            in self.ENV_VARS_TRUE_VALUES
        )
        self._mlflow_log_model_per_n_epochs = int(
            os.getenv("YOLOX_MLFLOW_LOG_MODEL_PER_n_EPOCHS", 30)
        )

        self._mlflow_log_nth_epoch_models = (
            os.getenv("YOLOX_MLFLOW_LOG_Nth_EPOCH_MODELS", "False").upper()
            in self.ENV_VARS_TRUE_VALUES
        )
        self.run_name = os.getenv("YOLOX_MLFLOW_RUN_NAME", None)
        self.run_name = None if len(self.run_name.strip()) == 0 else self.run_name
        self._flatten_params = (
            os.getenv("YOLOX_MLFLOW_FLATTEN_PARAMS", "FALSE").upper() in self.ENV_VARS_TRUE_VALUES
        )
        self._nested_run = (
            os.getenv("MLFLOW_NESTED_RUN", "FALSE").upper() in self.ENV_VARS_TRUE_VALUES
        )
    ...
    def convert_exp_todict(self, exp):
        ...
        exp_dict = {
            k: v for k, v in exp.__dict__.items() if not k.startswith("__") and k in filter_keys
        }
        return exp_dict
